...
function VolunteerView({ onModeChange }: { onModeChange: (mode: Mode) => void }) {
  ...
  const startNavigation = () => {
    ...
    navigator.geolocation.getCurrentPosition((position) => { setRouteOrigin({ lat: position.coords.latitude, lng: position.coords.longitude }); setRouteOpen(true); setToast("GPS locked · live traffic route to Zone B is active"); window.open("https://www.google.com/maps/dir/?api=1&destination=13.0831,80.2702&travelmode=walking", "_blank", "noopener,noreferrer"); }, () => setToast("GPS permission needed for live navigation"), { enableHighAccuracy: true, timeout: 5000 });
  };
